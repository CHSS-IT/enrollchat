namespace :scheduler do
  ...
  task :schedule_old_term_purge => :environment do
    # uncomment after first run
      ...
      @report = {}
      deleted_terms = Section.terms_to_delete.sort.each { |term| puts "#{term}"}.join("<br>")
  ...
  def send_report
    ...
    CommentsMailer.generic(email.html_safe, "Terms Marked for Deletion", ENV['ENROLLCHAT_ADMIN_EMAIL']).deliver!
  end
