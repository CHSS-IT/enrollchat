...
class UsersControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should not GET new user for an unregistered user' do
    unregistered_login
    get new_user_url
    assert_redirected_to unregistered_url
    assert_equal 'You are not registered to use this system.', flash[:notice]
  ...
  test 'should not create a new user with an unregistered user' do
    unregistered_login
    assert_no_difference('User.count') do
      post users_path, params: { user: { first_name: 'Timothy', last_name: 'McGee', email: 'tmcgee@test.com', username: 'tmcgee', admin: false } }
    end
  ...
  test 'should not UPDATE user for an unregistered user' do
    unregistered_login
    patch user_url(@user), params: { user: { first_name: 'New', last_name: 'Name' } }
    assert_redirected_to unregistered_url
    assert_not_equal @user.reload.first_name, 'New'
  ...
  test 'should not post to checked_activities for unregistered user' do
    unregistered_login
  ...
  test 'should not get archive for unregistered user' do
    unregistered_login
    get archive_user_url(@user)
    assert_redirected_to unregistered_url
    assert_equal 'You are not registered to use this system.', flash[:notice]
  ...
  test 'should not update user attributes for an unregistered user trying to archive a user' do
    unregistered_login
    @user.departments << 'HIST'
    @user.update(email_preference: 'Daily Digest')
    get archive_user_url(@user)
  ...
  test 'should remove admin status when archiving' do
    login_as(users(:one))
    @user.admin = true
    get archive_user_url(@user)
    assert_equal @user.reload.admin, false
