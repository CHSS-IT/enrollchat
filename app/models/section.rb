class Section < ApplicationRecord
  ...
  def self.import #(file)
    ...
    path = 'doc/SemesterEnrollments.csv'
    # Do any file cleanup/bulletproofing
    ...
    # Parse spreadsheet starting after header row
    (2..spreadsheet.last_row).each do |i|
      ...
      # TODO: to we have a flag for cancellation?

      # Save if changed, touch if unchanged
      section.changed? ? section.save! : section.touch
    end
    ...
    if touched.size > 0
      report_action('Updated Sections', "#{touched.size} sections were created or updated during the import process. Of those, #{created.size} are new.")
    else
